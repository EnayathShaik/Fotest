
<section id="main-slider" class="no-margin">
  <div class="carousel-inner innerpage">
    <div class="container">
      <div class="row slide-margin">
        <div class="col-sm-12">
          <h1 class="animation animated-item-1" style="padding-bottom:10px;">REGISTER HERE</h1>
        </div>
        <div class="circle-wrap center-block">
          <ul>
            <li><a href="trainee.fssai"><img src="website/images/trainee.png"></a></li>
            <li><a href="trainer.fssai"><img src="website/images/trainer.png"></a></li>
            <li><a href="trainingPartner.fssai"><img src="website/images/training-partner.png"></a></li>
            <li><a href="assessor.fssai"><img src="website/images/assessor.png"></a></li>
          </ul>
        </div>
      </div>
    </div>
    <!--/.item--> 
    <!--/.item--> 
    <!--/.item--> 
  </div>
  <!--/.carousel-inner--> 
  <!--/.carousel--> 
</section>
<!--/#main-slider--> 
<!--/#feature-->
<section id="study">
  <div class="row">
    <div class="col-md-12 col-sm-12 pull-left">
      <h1 style="padding-right:216px; border-bottom:1px #0b0c84 solid; padding-bottom:10px;">FoSTaC- Food Safety Training & Certification Process</h1>
    </div>
  </div>
  </div>
  <div class="row">
    <div class="col-sm-1 hidden-xs animation animated-item-4"></div>
    <div class="col-md-8 col-sm-12">&nbsp;</div>
  </div>
  <!-- timeline starts -->
  <div class="container">
    <div class="page-header">
      <p><strong>After availing each course you can get certificate on Food Safety which will be valid for two years. Follow the below steps to get Food Safety Certificatin:</strong></p>
    </div>
    <ul class="timeline">
      <li>
        <div class="timeline-badge success">1</div>
        <div class="timeline-panel">
          <div class="timeline-heading">
            <h4 class="timeline-title">Step 1</h4>
          </div>
          <div class="timeline-body">
            <p>Course Enrollment</p>
	    <p> <a href="login.fssai" style="color:#c52d2f;">Basic</a> | <a href="login.fssai" style="color:#c52d2f;">Advance</a> | <a href="login.fssai" style="color:#c52d2f;">Special</a></P>	
          </div>
        </div>
      </li>
      <li class="timeline-inverted">
        <div class="timeline-badge success">2</div>
        <div class="timeline-panel">
          <div class="timeline-heading">
            <h4 class="timeline-title">Step-2</h4>
          </div>
          <div class="timeline-body">
            <p>Get Your Admit Card</p>
          </div>
        </div>
      </li>
      <li>
        <div class="timeline-badge success">3</div>
        <div class="timeline-panel">
          <div class="timeline-heading">
            <h4 class="timeline-title">Step-3</h4>
          </div>
          <div class="timeline-body">
            <p>Attend Training</p>
            <p><a href="login.fssai" style="color:#c52d2f;">Online</a> | <a href="login.fssai" style="color:#c52d2f;">Classroom</a></p>
          </div>
        </div>
      </li>
      <li class="timeline-inverted">
        <div class="timeline-badge success">4</div>
        <div class="timeline-panel">
          <div class="timeline-heading">
            <h4 class="timeline-title">Step-4</h4>
          </div>
          <div class="timeline-body">
            <p>Get Your Self Assessed</p>
            <p><a href="login.fssai" style="color:#c52d2f;">Online</a> | <a href="login.fssai" style="color:#c52d2f;">In Physical Center</a></p>
          </div>
        </div>
      </li>
      <li>
        <div class="timeline-badge success">5</div>
        <div class="timeline-panel">
          <div class="timeline-heading">
            <h4 class="timeline-title">Step-5</h4>
          </div>
          <div class="timeline-body">
            <p>Give Your Feedback</p>
          </div>
        </div>
      </li>
      <li class="timeline-inverted">
        <div class="timeline-badge success">6</div>
        <div class="timeline-panel">
          <div class="timeline-heading">
            <h4 class="timeline-title">Step-6</h4>
          </div>
          <div class="timeline-body">
            <p>Get Your Certificate</p>
          </div>
        </div>
      </li>
    </ul>
  </div>
  <!-- timeline ends --> 
</section>
</div>